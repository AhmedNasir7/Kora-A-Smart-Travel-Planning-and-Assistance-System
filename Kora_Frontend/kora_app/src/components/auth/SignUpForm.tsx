  ...
  return (
    <div className="flex h-full flex-col animate-slide-up-md text-[#f5f3eb]">
      <div className="space-y-2.5">
        ...
        <div className="space-y-1 text-center">
          ...
          <p className="text-[14px] font-normal text-[#a9a59b]">
            Sign up to kick start your journey
          </p>
        </div>
      </div>
      ...
      <div className="mt-3 flex border-b border-[#2a3344] text-[13px]">
        <Link
          href="/signin"
          className="px-1 pb-2.5 font-semibold text-[#7c808a] transition-colors hover:text-[#f5f3eb]"
        >
          {authConfig.labels.signIn}
        </Link>
        <button className="relative ml-6 px-1 pb-2.5 font-semibold text-[#f5f3eb]">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-3 space-y-2">
        ...
        <FormInput
          label={authConfig.labels.username}
          placeholder={authConfig.placeholders.username}
        ...
        >
          {isLoading ? 'Creating account...' : authConfig.labels.signUp}
        </button>
        ...
        <OAuthButton
          ...
          onClick={async () => {
            ...
            if (!result.ok) {
              setApiError(result.error || 'Google sign up failed. Please try again.');
            }
          }}
        >
      ...
      <p className="mt-3 text-center text-[13px] text-[#7c808a]">
        ...
        <Link
          href="/signin"
          className="ml-1 text-[#ff6b35] font-semibold transition-colors hover:text-[#ff8a5b]"
        >
          Sign In
        </Link>
      </p>
